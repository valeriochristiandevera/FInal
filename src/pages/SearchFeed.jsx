...
<h3 className='text-md sm:text-lg md:text-xl traking-wide font-normal leading-[19px] sm:leading-[22px] md:leading-[24px]' style={{ color: colors.textPrimary }}>
                    {e.snippet?.title}
                  </h3>
                  <span className='text-[12px]' style={{ color: colors.textSecondary }}>{timeSince(new Date(Date.parse(e.snippet?.publishedAt) - aDay))}</span>
                  <h4 onClick={() => pageRoute(`/channel/${e.snippet?.channelId}`)} className='font-medium text-[12px] sm:my-1' style={{ color: colors.textSecondary }}>{e.snippet?.channelTitle}</h4>
                  <p onClick={() => pageRoute(`/watch/${e.id?.videoId}`)} className='traking-wider font-normal text-[10px] text-[13px] leading-[16px]' style={{ color: colors.textPrimary }}>{e.snippet?.description?.slice(0, 124) + "..."}</p>
                </div>
              </div>
            )
          })
